    ...
    <main id="main-container" class="container py-5">
        <div class="row">
            <div class="col-12">
                ...
                <div class="row">
                    <div id="users-box" class="col-12 col-lg-10 col-xl-8 mx-auto">
                        <h4 class="users-box-label mb-4">Utilizadores</h4>

                        <?php

                        // ----- USERS -----

                        // Usar prepared statement para ir buscar os dados dos utilizadores
                        $tipo = "cliente";
                        $stmt = $conn->prepare("SELECT id, nome, apelido, dataNascimento, telefone, email FROM utilizadores WHERE tipo = ? ORDER BY nome");
                            ...
                            <table class="table table-hover table-fixed" id="users-table">
                                <thead>
                                    <tr class="table-primary">
                                        <th scope="col" class="d-table-cell">Nome</th>
                                        <th scope="col" class="d-none d-md-table-cell">Data Nasc.</th>
                                        <th scope="col" class="d-none d-md-table-cell">Telefone</th>
                                        <th scope="col" class="d-table-cell">E-mail</th>
                                        <th scope="col" class="d-table-cell text-center">Ver perfil</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    if ($result->num_rows === 0) {
                                        echo "<tr><td colspan='5'>Não há utilizadores registados.</td></tr>";
                                    } else if ($result->num_rows > 0) {
                                        while ($userList = $result->fetch_assoc()) {
                                            echo "<tr>";
                                            echo "<td class='d-table-cell'>" . $userList['nome'] . " " . $userList['apelido'] . "</td>";
                                            echo "<td class='d-none d-md-table-cell'>" . date('d-m-Y', strtotime($userList['dataNascimento'])) . "</td>";
                                            echo "<td class='d-none d-md-table-cell'>" . $userList['telefone'] . "</td>";
                                            echo "<td class='d-table-cell'>" . $userList['email'] . "</td>";
                                            echo "<td class='d-table-cell text-center'><a href='$BASE_URL/profile_user.php?id=" . $userList['id'] . "'><i class='fa-solid fa-user'></i></a></td>";
                                        }
                                    ?>
                                </tbody>
                            </table>
